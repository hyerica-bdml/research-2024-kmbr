...
//        kmbr.printDirtyProbs();
        ...
        try (FileWriter fout = new FileWriter(String.format("logs/cacheTest/K%d_N%d_without_caching.csv", Config.K, Config.NUM_OF_POINTS));
        ...
        try (FileWriter fout = new FileWriter(String.format("logs/cacheTest/K%d_N%d_with_caching.csv", Config.K, Config.NUM_OF_POINTS));
             BufferedWriter bout = new BufferedWriter(fout)) {
            long startTime = System.currentTimeMillis();
            MBR mbr = kmbr.find();
            double runtime = (System.currentTimeMillis() - startTime) / 1000.0;
