...
public class CovidSimulationApp {
    ...
    public static void main(String[] args) {
        new CovidSimulationApp().run(
                ...
                String.format("logs/related_works/enclosing1998/covid_american_streaming_k%d_2020-1.txt", Config.K)
        );

        new CovidSimulationApp().run(
                "data/covid-19-in-the-american-continent/parsed/covid_american_streaming_2020-7.txt",
                String.format("logs/related_works/enclosing1998/covid_american_streaming_k%d_2020-7.txt", Config.K)
        );

        new CovidSimulationApp().run(
                "data/covid-19-in-the-american-continent/parsed/covid_american_streaming_2021-1.txt",
                String.format("logs/related_works/enclosing1998/covid_american_streaming_k%d_2021-1.txt", Config.K)
        );

        new CovidSimulationApp().run(
                "data/covid-19-in-the-american-continent/parsed/covid_american_streaming_2021-7.txt",
                String.format("logs/related_works/enclosing1998/covid_american_streaming_k%d_2021-7.txt", Config.K)
        );
    }
